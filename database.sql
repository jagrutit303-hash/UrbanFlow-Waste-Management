DROP DATABASE IF EXISTS urbanflow_db;
...
-- 7. driver_locations table (NEW — for real-time GPS tracking)
...
-- Users: Citizen, Collector/Driver, Admin
...
-- Zones with coordinates (Davanagere Area)
...
-- Driver location (simulate Driver Dave near South District)
INSERT INTO driver_locations (driver_id, lat, lng) VALUES 
(2, 40.7150, -74.0080);

-- Illegal dump reports (mock data)
INSERT INTO illegal_dumps (citizen_id, zone_id, description, severity, status, citizen_lat, citizen_lng) VALUES 
(1, 4, 'Large pile of construction debris dumped on sidewalk near the park entrance.', 'High', 'Reported', 40.7010, -74.0190),
(1, 1, 'Bags of household trash left outside dumpster area.', 'Medium', 'Under Review', 40.7130, -74.0055);
